...
def main():
    ...
    while choice != MENU_QUIT:
        ...
        elif choice == MENU_RECOMMEND:
            recommend_album(albums)
        elif choice == MENU_ADD:
            add_album(albums)
        elif choice == MENU_MARK:
...
def mark_album_completed(albums):
    """Mark a required album as completed."""
    if count_required_albums(albums) == 0:
        print("No required albums.")
        return
    display_albums(albums)
    print("Enter the number of an album to mark as completed")
    album_number = get_valid_album_number(len(albums))
    album = albums[album_number - 1]
    if album[STATUS_INDEX] == COMPLETED:
        print("You have already completed {}".format(album[TITLE_INDEX]))
    else:
        album[STATUS_INDEX] = COMPLETED
        print("{} by {} completed!".format(
            album[TITLE_INDEX], album[ARTIST_INDEX]))
...
def get_valid_album_number(number_of_albums):
    """Get a valid album number from the user."""
    album_number = get_positive_integer(">>> ")
    while album_number > number_of_albums:
